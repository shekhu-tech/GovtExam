...
// FIXED: Google Sheets data submission
...
(function() {
    ...
    function generateRegistrationId(name, phone, email, formType) {
        ...
        // Fix: emailPart was not defined - using emailLocal
    ...
    function initRazorpay(formType, name, phone, email) {
        const options = {
            ...
            handler: function(response) {
                ...
                document.getElementById('successCustomerName').textContent = name.split(' ')[0];
    ...
    async function sendToSheet(data) {
        ...
        params.append('formType', data.formType);
        params.append('name', data.name);
        params.append('number', data.phone);
        params.append('email', data.email);
        params.append('amount', data.amount);
        params.append('plan_name', data.plan_name);
        params.append('transaction_id', data.txn);
        params.append('registration_id', data.registrationId);
        params.append('timestamp', data.time);
        params.append('app_package', APP_PACKAGE_NAME);
        
        try {
            // Method 1: Using fetch with CORS mode (recommended)
            ...
            // Fallback: Try with image/pixel method (always works)
            fallbackSendToSheet(data);
        }
    }
    
    // Fallback method using image pixel (100% reliable)
    function fallbackSendToSheet(data) {
        const url = 'https://script.google.com/macros/s/AKfycbz1eAo0xc9l_-_DluIoNxT6XzTjrKOoPqYgWBXw_gHYO-IWHndaBLtKlJ8Lc9Q8uHqceg/exec';
        ...
        console.log('Fallback method used for Google Sheets');
    }
